...
const tools = {
  searchClubs: {
    ...
    execute: async ({ category, audience, minMembers }: { category?: string; audience?: string; minMembers?: number }) => {
      let results = [...clubsDb];
  ...
  escalateToHuman: {
    ...
    execute: async ({ reason, brandName, budget, summary }: { reason: string; brandName?: string; budget?: number; summary: string }) => {
      const lead = {
